...
public interface AlbumRepository extends JpaRepository<Album, Integer>
{
    ...
    @Query(value = "SELECT art.id as artistId, art.name as artistName, alb.id as id, alb.title as title "
                    + "FROM user_favorite_album ufa, album alb, artist art, album_artist_mapping aam "
                    + "WHERE ufa.album_id = alb.id AND ufa.user_id = :userId AND aam.artist_id = art.id AND aam.album_id = alb.id "
